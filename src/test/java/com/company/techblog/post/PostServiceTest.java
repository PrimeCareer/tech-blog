...
class PostServiceTest {
    ...
    void getPopularPosts() {
        ...
        Post normalPost = Post.builder()
            ...
            .content("일반글 내용")
            .build();
        given(postRepository.findTop10ByOrderByLikeCountDescCreatedAtDesc()).willReturn(List.of(popularPost, normalPost));

        // when
        List<Response> popularPosts = postService.getPopularPosts();

        // then
        assertThat(popularPosts).hasSize(2);
        assertThat(popularPosts.get(0).getTitle()).isEqualTo("인기글");
        assertThat(popularPosts.get(1).getTitle()).isEqualTo("일반글");
        verify(postRepository).findTop10ByOrderByLikeCountDescCreatedAtDesc();
    }
}